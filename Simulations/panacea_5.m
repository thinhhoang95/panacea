function [dbg_meas,dbg_ofd,dbg_csr,dbg_acc,X_out,X_true,P_out,accuracy] = panacea_5(Aa, Add, Ba, Bdd, imuTs, trackingTs, out, N_window, Q, R_val, f, accel_rndprc)
    close all;
    ...
    for i=1:length(out.ofd.time)
        ...
        for lm=1:length(out.ofd.signals.values(:,:,i))
            out.ofd.signals.values(lm,:,i) = out.ofd.signals.values(lm,:,i) + (-pixelVar*0.059375e-3+2*rand([1 2])*pixelVar*0.059375e-3);
        end
    end
    ...
    for time=1:time_max % For each IMU timestep, time is the multiplier of imuTs
        ...
        if (clock>next_tracking_clock && clock>3)
            ...
            Ra(1:length(ofdx)*2,1:length(ofdx)*2) = 1e-6 * R_val * eye(length(ofdx)*2);
            ...
            Ra(length(ofdx)*2+tracking_delay_length*3+1:length(ofdx)*2+tracking_delay_length*3+6,length(ofdx)*2+tracking_delay_length*3+1:length(ofdx)*2+tracking_delay_length*3+6)=1e-12 * eye(6);
