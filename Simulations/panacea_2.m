function [dbg_meas,dbg_ofd,dbg_csr,dbg_acc] = panacea_2(Aa, Add, Ba, Bdd, imuTs, trackingTs, out, N_window, Q, R, f)
    ...
    for time=1:time_max % For each IMU timestep, time is the multiplier of imuTs
        ...
        if (clock>next_tracking_clock && clock>3)
            ...
            for i=1:length(ofdx)
                ...
                z((i-1)*2+1:i*2) = F*Y_f*((-x_i(3)/f) * inv(Y_i) * [xe ye 0]' + [x_f(1) x_f(2) 0]');
            ...
            Ra(1:length(ofdx)*2,1:length(ofdx)*2) = R * eye(length(ofdx)*2);
